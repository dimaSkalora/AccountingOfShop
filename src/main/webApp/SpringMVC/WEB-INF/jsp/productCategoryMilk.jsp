        ...
        <table>
            ...
            <tr>
                <td colspan="2">
                    <c:if test="${!empty product.id}">
                        <input type="submit"
                               value="<spring:message text="Update Product Milk"/> "/>
                    </c:if>
                    <c:if test="${empty product.id}">
                        <input type="submit"
                               value="<spring:message text="Add Product Milk"/> "/>
                    </c:if>
                </td>
            </tr>
        </table>
    ...
    <table border="1" cellpadding="8" cellspacing="0">
        ...
        <c:forEach items="${categoryMilk}" var="product">
            ...
            <tr class="${product.balanceNegative ? 'reduced' : 'normal'}">
                ...
                <td><a href="<c:url value="/productCategoryMilk/update?id=${product.id}"/>"/>update</td>
                <td><a href="<c:url value="/productCategoryMilk/delete?id=${product.id}"/>"/>delete</td>
            </tr>
        </c:forEach>
    </table>
</section>
