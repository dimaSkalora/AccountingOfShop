        ...
        <table>
            ...
            <tr>
                <td colspan="2">
                    <c:if test="${!empty cigarette.id}">
                        <input type="submit"
                               value="<spring:message text="Update Cigarette Without Filter"/> "/>
                    </c:if>
                    <c:if test="${empty cigarette.id}">
                        <input type="submit"
                               value="<spring:message text="Add Cigarette Without Filter"/> "/>
                    </c:if>
                </td>
            </tr>
        </table>
    ...
    <table border="1" cellpadding="8" cellspacing="0">
        ...
        <c:forEach items="${categoryWithoutFilter}" var="cigarette">
            ...
            <tr class="${cigarette.balanceNegative ? 'reduced' : 'normal'}">
                ...
                <td><a href="<c:url value="/cigaretteCategoryWithoutFilter/update?id=${cigarette.id}"/>"/>update</td>
                <td><a href="<c:url value="/cigaretteCategoryWithoutFilter/delete?id=${cigarette.id}"/>"/>delete</td>
            </tr>
        </c:forEach>
    </table>
</section>
